	...
	*/
    ...
    if (HISTOGRAMS_ON) {
      ...
      PlotAll1D();
	    ...
	    Bool_t sort_on = kTRUE;
	    Bool_t LR_mode = kFALSE;
	    PlotAllFactorization();
	    PlotAll("Detector", LR_mode, sort_on);
	    PlotAll("Generator", LR_mode, sort_on);
    }
...
std::vector<Double_t> CombinatoricsSuper::EMsub(std::vector<CrossSection>& xs, Int_t data_index, Int_t MC_index, Bool_t final_round, UInt_t EXTRACTION_LEVEL) {
	...
	for (Int_t j = 0; j < C_; ++j) {    	// Loop over processes
		...
		effVec[j] = 1 - F.at(0).at(j); 		// Save the efficiency
		...
		if (!(sum > EPS)) {
			effVec[j] = 0; // This process is null (MC does not include it)
		}
	}
