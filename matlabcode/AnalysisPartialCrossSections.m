...
%param.comb_level = 1;   % Detector level
param.comb_level = 2;    % Fiducial (unfolded particle) level

% Unfolding iteration range
param.hih = 8;           % Maximum iteration
param.cen = 2;           % Central value
param.low = 1;           % Minimum iteration
...
param.sigma_inel_MC  = 79;                    % Total inelastic in MC
...
for model = 1
    param.model = model;
    [mins, acceptances, METRICS] = Combinatorial(param, PLOT_ON, DATA_ON);
end
...
text(mu_fid*0.978,  0.88, '$\sigma_{\mathrm{fid}}^{measured}$','interpreter','latex','fontsize',22);
...
text(mu_fid*0.982,  0.80, 'Luminosity','interpreter','latex','fontsize',10);
